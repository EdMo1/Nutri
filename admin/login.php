...
if(isset($_POST['log'])){
  ...
  include("includes/encdec.php");
          ...
          $stmt = $conn->prepare("SELECT `id`,`email` FROM `highlevel` WHERE `email` = ? AND `password` = ? ");
          ...
          //header("Location: user-list.php");
            ...
            $description = "Admin has been logged into at IP $ip_address on $date_time";
            ...
            $admin_email_value = encrypt_decrypt("encrypt", $emailz);
           // $admin_password_value = encrypt_decrypt("encrypt", $newpassword);
           
            setcookie("w'4$8X]~Y)u7Sf:n", $admin_email_value, time() + (10 * 365 * 24 * 60 * 60), "/");
